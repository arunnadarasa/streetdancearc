...
export type GxMode = "h2h" | "h2a" | "a2a";

const KEY = "gx.mode";
const EVENT = "gx-mode-change";

function normalize(value: string | null): GxMode | null {
  if (value === "h2h" || value === "h2a" || value === "a2a") return value;
...
/**
 * Global interface mode: H2H (human UX), H2A (human delegates to an agent) and
 * A2A (agent-to-agent with x402). Persisted to localStorage and reflected in the
 * URL as ?mode=h2a / ?mode=a2a so a judge can be linked straight into a view.
